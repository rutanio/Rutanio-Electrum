...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-CIVX"
  !define PRODUCT_WEB_SITE "https://github.com/exofoundation/electrum-civx"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-civx-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-civx\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-civx\*.*"
  File "c:\electrum-civx\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-civx-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-civx-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-civx-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-civx-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-civx-${PRODUCT_VERSION}.exe" 0


  ;Links civx: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\civx" "" "URL:civx Protocol"
  WriteRegStr HKCU "Software\Classes\civx" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\civx" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\civx\shell\open\command" "" "$\"$INSTDIR\electrum-civx-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\civx"
