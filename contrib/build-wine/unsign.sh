...
version=`python3 -c "import electrum_civx; print(electrum_civx.version.ELECTRUM_VERSION)"`
...
for mine in $(ls dist/*.exe); do
    echo "---------------"
    f=$(basename $mine)
    echo "Downloading https://download.civxeconomy.com/$version/$f"
    wget -q https://download.civxeconomy.com/$version/$f -O signed/$f
